...
<form action="<?=HOST;?>edit-post&amp;id=<?=$post->getId()?>" method="post" enctype="multipart/form-data">
        ...
        <div class="md-form">
            ...
            <input type="text" id="name"   name="name"  class="form-control" value="<?=htmlspecialchars($post->getName())?>">
    ...
    <div class="md-form">
        ...
        <div class="md-form">
            ...
            <textarea type="text" id="content" name="content" class="form-control md-textarea" rows="3"><?=htmlspecialchars($post->getContent())?></textarea>
            <label for="content"</label>
        </div>

        <input type="hidden" name="MAX_FILE_SIZE" value="1024000000" />
        <input type="file" name="img" id="file_upload" value="" />
        <input type="hidden" name="img" value="<?=$post->getImg()?>"/>
        <img src="<?=ASSETS;?>images/<?=$post->getImg()?>" id="image_uploaded" alt ="image" style="width: 150px; height:90px; ">
